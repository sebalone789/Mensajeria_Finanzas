...
import finanzas.p.e.mensajeriaepfinanzas.Control.FileManager.EventHandler;
import finanzas.p.e.mensajeriaepfinanzas.Control.FileManager.FileManager;
import finanzas.p.e.mensajeriaepfinanzas.Control.FileManager.Settings;
...
public class BuscarArchivo extends ListActivity {
    ...
    private TextView mPathLabel,mStorageLabel,mDetailLabel;

    //Valores de la insercion
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_buscar_archivo);
        ...
        if (savedInstanceState != null)
            mHandler = new EventHandler(BuscarArchivo.this, mFileMag, savedInstanceState.getString("location"));
        else
            mHandler = new EventHandler(BuscarArchivo.this, mFileMag);
	    	...
	    	/* generic intent */
                ...
                if(file.exists()) {
                    if(mReturnIntent) {
                        ...
                        try {
                            cargarDatosExcel(this.getApplicationContext(),Uri.fromFile(file).getPath());
                        } catch(ActivityNotFoundException e) {
                            Toast.makeText(this, "No se encontro el archivo" + file.getName()+" para abrir",
                                    Toast.LENGTH_SHORT).show();
                        }
                    }
                }
    ...
    private void cargarDatosExcel(Context context, String pathname ) {

        //Valores para la busqueda
        Integer idGrupo=0;
        String nombre="", apPaterno="", apMaterno="", numero="", grupo="";
        ...
        if(inputWorkbook.exists()){
            ...
            try {
                ...
                for (int j = 0; j < sheet.getRows(); j++) {
                    ...
                    //Valores para ir ingresando
                    ContentValues values = new ContentValues();
                    //Leer los valores por columnna
                    for (int i = 0; i < sheet.getColumns(); i++) {
                        Cell cel = sheet.getCell(i, j);
                        switch (i)
                        {
                            case 0:
                                values.put(MensajeriaContract.Persona.nombreColumnaNombre,cel.getContents());
                                nombre=cel.getContents();
                                break;
                            case 1:
                                values.put(MensajeriaContract.Persona.nombreColumnaApPaterno,cel.getContents());
                                apPaterno=cel.getContents();
                                break;
                            case 2:
                                values.put(MensajeriaContract.Persona.nombreColumnaApMaterno,cel.getContents());
                                apMaterno=cel.getContents();
                                break;
                            case 3:
                                values.put(MensajeriaContract.Persona.nombreColumnaNumero,cel.getContents());
                            ...
                            case 4:
                                ...
                                Cursor c2=db.rawQuery("SELECT * FROM "+MensajeriaContract.Grupo.nombreTabla+" WHERE "+
                                        MensajeriaContract.Grupo.nombreColumnaNombreGrupo+" LIKE '"+cel.getContents()+"'",null);
                                if(c2!=null&&c2.moveToFirst())
                                {
                                    values.put(MensajeriaContract.Persona.nombreColumnaGrupo,c2.getInt(c2.getColumnIndex(MensajeriaContract.Grupo._ID)));
                                    idGrupo=c2.getInt(c2.getColumnIndex(MensajeriaContract.Grupo._ID));
                                }
                                else {
                                    ContentValues val2=new ContentValues();
                                    val2.put(MensajeriaContract.Grupo.nombreColumnaNombreGrupo,cel.getContents());
                                    long grupoNuevo=db.insert(MensajeriaContract.Grupo.nombreTabla,null,val2);
                                    Cursor c3=db.rawQuery("SELECT * FROM "+MensajeriaContract.Grupo.nombreTabla+" WHERE "+
                                            MensajeriaContract.Grupo.nombreColumnaNombreGrupo+" LIKE '"+cel.getContents()+"'",null);
                                    if(c3!=null&&c3.moveToFirst())
                                    {
                                        values.put(MensajeriaContract.Persona.nombreColumnaGrupo,c3.getInt(c3.getColumnIndex(MensajeriaContract.Grupo._ID)));
                                        idGrupo=c3.getInt(c3.getColumnIndex(MensajeriaContract.Grupo._ID));
                                    }
                                }
                                break;
                            default:
                                System.out.println("Tiene mas de 5 Columnas");
                                break;
                        }
                    }
                    //Verificar que el registro no exista
                    Cursor c = db.rawQuery("SELECT * FROM "+ MensajeriaContract.Persona.nombreTabla+" WHERE "+
                            MensajeriaContract.Persona.nombreColumnaNombre+" LIKE '"+nombre+"' AND "+
                            MensajeriaContract.Persona.nombreColumnaApPaterno+" LIKE '"+apPaterno+"' AND "+
                            MensajeriaContract.Persona.nombreColumnaApMaterno+" LIKE '"+apMaterno+"' AND "+
                            MensajeriaContract.Persona.nombreColumnaNumero+" LIKE '"+numero+"' AND "+
                            MensajeriaContract.Persona.nombreColumnaGrupo+" LIKE "+idGrupo,null);
        ...
        {
            Toast.makeText(this, "No es un archivo excel",
                    Toast.LENGTH_SHORT).show();
        }
        finish();
    }
...
//    	menu.add(0, MENU_SPACE, 0, "Free space").setIcon(R.drawable.space);
    ...
    protected Dialog onCreateDialog(int id) {
        final Dialog dialog = new Dialog(BuscarArchivo.this);
                    ...
                    public void onClick (View v) {
                        if (input.getText().length() > 1) {
                            if (mFileMag.createDir(mFileMag.getCurrentDir() + "/", input.getText().toString()) == 0)
                                Toast.makeText(BuscarArchivo.this,
                                        "Folder " + input.getText().toString() + " created",
                                        Toast.LENGTH_LONG).show();
                            else
                                Toast.makeText(BuscarArchivo.this, "New folder was not created", Toast.LENGTH_SHORT).show();
                        }
                ...
                rename_create.setOnClickListener(new View.OnClickListener() {
                    public void onClick (View v) {
                        ...
                        if(mFileMag.renameTarget(mFileMag.getCurrentDir() +"/"+ mSelectedListItem, rename_input.getText().toString()) == 0) {
                            Toast.makeText(BuscarArchivo.this, mSelectedListItem + " was renamed to " +rename_input.getText().toString(),
                                    Toast.LENGTH_LONG).show();
                        }else
                            Toast.makeText(BuscarArchivo.this, mSelectedListItem + " was not renamed", Toast.LENGTH_LONG).show();
    ...
    public boolean onKeyDown(int keycode, KeyEvent event) {
        ...
        if(keycode == KeyEvent.KEYCODE_SEARCH) {
            ...
            if(mHandler.isMultiSelected()) {
                ...
                Toast.makeText(BuscarArchivo.this, "Multi-select is now off", Toast.LENGTH_SHORT).show();
            ...
            Toast.makeText(BuscarArchivo.this, "Press back again to quit.", Toast.LENGTH_SHORT).show();
